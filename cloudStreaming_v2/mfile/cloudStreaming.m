...
for cn = 1 : length(bw_setting)
    dynamic_bw = [dynamic_bw get_exp_bw(total_time, bw_setting(cn), exp_num, cn)];
end
...
while i < vid_len
    if strcmp(state, 'Buffering')
        ...
        for c = 1 : conn_num
            ...
            [~, rec_TS(c)] = chunk_download(chunk_size, chunk_play_seq, dynamic_bw(:, c), TS, cur_play_TS, 0);
    ...
    elseif strcmp(state, 'Steady') 
        ...
        if (curBufSz <= bufSz - chunkLen)
            ...
            for c = 1 : conn_num
                ...
                chunk_size = frame_size(download_frm_idx{c});
                chunk_play_seq = play_seq(download_frm_idx{c});
                [dropped_frms, rec_TS(c)] = chunk_download(chunk_size, chunk_play_seq, dynamic_bw(:, c), TS, cur_play_TS, drop_idx);
                all_dropped_frames = [all_dropped_frames download_frm_idx{c}(dropped_frms)];

                % disp(['Downloading chunk with starting frame ' num2str(i) ' at connection ' num2str(c) ' !!!']);
            ...
            dropEvents = [dropEvents; all_dropped_frames' dropped_impIdx dropped_frame_size dropped_frame_type];
