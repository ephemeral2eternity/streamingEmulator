...
% close all;
% clc;
function adaptiveStreaming(vid_name, exp_num, chunkLen, start_cnt, bw_setting, profile_kbps)
...
while i < vid_len
    if strcmp(state, 'Buffering')
        download_frm_idx = i : i + start_cnt * chunkLen * frmRate - 1;
        ...
        for s = 1 : start_cnt
            ...
            disp(['Downloading chunk ' num2str(chunk_id) ' with profile ' num2str(profile_kbps(profile_idx)) '(kbps) at frame ' num2str(i) ' !!!']);
            chunk_id = chunk_id + 1;
        end
        
        % Estimate the available bandwidth for last request.
        [predict_bw, ~] = emulate_monitoring(dynamic_bw, rec_TS);
    ...
    elseif strcmp(state, 'Steady') 
        ...
        if (curBufSz > 0)
            if (curBufSz <= theta_1)
                ...
                % possible with the lowest profile.method_ind
                profile_idx = profile_num;
                ...
                preTS = TS;                
                TS = rec_TS;
            elseif (curBufSz <= theta_2)
                % Enter "Low_Buf" mode, to download a chunk with the lower
                % possible profile
                if (profile_idx < profile_num)
                    profile_idx = profile_idx + 1;
                    % download_profiles(download_frm_idx) = profile_idx;
                end        
            ...
            elseif (curBufSz <= bufSz - chunkLen)
                % Enter the "Comp_Profile" mode, to optimally choose a
                % profile that matches the estima2ted bitrate
                satisfied_prof_idx = find(predict_bw - profile_byterates > 0);        
                if isempty(satisfied_prof_idx)
                    profile_idx = profile_num;
                else
                    profile_idx = satisfied_prof_idx(1);
                end
                ...
                % Writing the log
                % disp(['======= downloading chunk ' num2str(i) ' with the bitrate ' num2str(profile_kbps(profile_idx))]);
